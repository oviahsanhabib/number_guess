...
BEST_GAME=$($PSQL "SELECT MIN (best_game) FROM games INNER JOIN users USING(user_id) WHERE username = '$USERNAME'")
...
INSERT_GAME=$($PSQL "INSERT INTO games (best_game, user_id ) VALUES ($GUESS, $USER_ID )")